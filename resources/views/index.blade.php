        ...
        <nav id="navBar">
            ...
            <div class="account-customer" id="accountCustomer">
                @if(isset($customerName))
                    <a style="color: white">{{ $customerName }}</a>
    ...
    <div class="container" style="margin-top: -18%">
        ...
        <div id="carousel-exclusives">
            <div class="exclusives">
                ...
                <div>
                    <a style="text-decoration: none; color: inherit;" href="{{ URL::to('/tourtype-detail/'.$tourtype->tourtype_id) }}">
                        <img class="exclusives-image" src="public/uploads/tourtype/{{ $tourtype -> tourtype_image }}">
                        <h3 class="name" style="text-align: center">{{ $tourtype->tourtype_name }}</h3>
                    </a>
        ...
        <div id="carousel-trending">
            ...
            <div class="trending">
                ...
                <div>
                    <a style="text-decoration: none; color: inherit;" href="{{ URL::to('/trending-place/'.$pl->place_id) }}">
                        <img class="trending-image" src="public/uploads/place/{{ $pl -> place_image }}">
                        <h3 class="name" style="text-align: center">{{ $pl->place_name }}</h3>
                    </a>
                </div>
                @endif
                @endforeach
        ...
        <div id="carousel-stories">
            ...
            <div class="stories">
                @foreach($all_tourdetail as $key => $tourdetail )
                    ...
                    @if($tourdetail->tour_id == $tour->tour_id)
        ...
        {{-- <a href="{{ URL::to('/tourtype-detail') }}" class="start-btn">Start making money</a> --}}
...
{{-- <script>
    const recognition = new webkitSpeechRecognition() || speechRecognition();
    recognition.lang = 'vi-VN'; // Ngôn ngữ, bạn có thể thay đổi theo yêu cầu

    recognition.onresult = function(event) {
        const result = event.results[0][0].transcript;
        document.getElementById('keywords_submit').innerText = result;
    }
