...
<body>
    ...
    <div class="container">
        <div class="list-container">
            ...
            <div class="right-col">
                {{-- <p>200+ Options</p> --}}
                @foreach($trending_place as $key => $trend_pl)
                <h1>{{ $trend_pl->place_name }}</h1>
                <p>{!! $trend_pl->place_describe !!}<br></p>
                    @foreach($all_destination_detail as $key => $des_dtl)
                        @foreach($all_destination as $key => $des)
                            @foreach($all_tourdetail as $key => $tourdetail)
                                ...
                                @if($trend_pl->place_id == $des_dtl->place_id && $des_dtl->destination_id == $des->destination_id && $des->destination_id == $tour->tour_destination && $tour->tour_id == $tourdetail->tour_id)
                                <div class="house">
                                    <div class="house-img">
                                    ...
                                    <div class="house-info">
                                        ...
                                        <p><i class="fa-solid fa-location-dot"></i><b> Các điểm đến:</b> 
                                            ...
                                            @foreach($all_place as $key => $pl)
                                                @foreach($all_destination_detail as $key => $des_dtls)
                                                    @if($pl->place_id == $des_dtls->place_id && $des_dtls->destination_id == $tour->tour_destination)
                                                        @if($firstPlace)
                                                            {{ $pl->place_name }}
